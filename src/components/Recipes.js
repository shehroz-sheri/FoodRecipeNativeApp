    ...
    return (
        <Animated.View entering={FadeInDown.delay(i + 100).duration(600).springify().damping(12)}>
            <Pressable style={{ width: '100%', paddingLeft: isEven ? 0 : 8, paddingRight: isEven ? 8 : 0 }} className='flex justify-center mb-4 space-y-1' onPress={() => navigation.navigate('RecipeDetail', { ...item })}>
                <Image source={{ uri: item.strMealThumb }} sharedTransitionTag={item.strMeal} style={{ width: '100%', height: i % 3 == 0 ? hp(25) : hp(35), borderRadius: 35 }} className='bg-black/5' />

                {/* <CachedImage uri={item.strMealThumb} style={{ width: '100%', height: i % 3 == 0 ? hp(25) : hp(35), borderRadius: 35 }} className='bg-black/5' /> */}
